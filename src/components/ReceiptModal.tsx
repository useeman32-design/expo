...
export function ReceiptModal({
  ...
  // share the receipt as a JPG image (native share sheet / web download);
  // if capture fails, fall back to copying the details
  const share = async () => {
    const res = await shareViewAsJpg(receiptRef, `stocksx-receipt-${tx.reference}.jpg`);
    if (res === 'failed') await copyFallback();
  };
        ...
        <Pressable style={styles.card} onPress={() => undefined}>
          ...
          <View style={styles.actions}>
            <Pressable onPress={share} style={({ pressed }) => [styles.shareBtn, pressed && { opacity: 0.85 }]}>
              ...
              <Text style={styles.shareBtnText}>{copied ? 'Copied!' : 'Share as image'}</Text>
            </Pressable>
...
const makeStyles = () => StyleSheet.create({
  ...
  // capture target: painted onto the card itself so the exported JPG looks
  // exactly like the on-screen receipt (background + radius carried over)
  capture: {
    margin: -S.xl,
    padding: S.xl,
    backgroundColor: C.surface,
